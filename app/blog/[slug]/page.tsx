...
import { getBlogPost, getBlogPostsForListing } from "@/lib/db" // Use optimized functions
...
export async function generateStaticParams() {
  try {
    // Use the optimized listing function
...
export async function generateMetadata({ params }: BlogPostParams): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      authors: [post.author?.name || "Khanna and Associates"],
      tags: post.tags,
    },
  }
}
...
export default async function BlogPostPage({ params }: BlogPostParams) {
  // Fetch the main post and related posts in parallel
  const [post, allPostsListing] = await Promise.all([
    getBlogPost(params.slug),
    getBlogPostsForListing() // Use optimized function
  ])
  ...
  // Filter related posts more efficiently
  const relatedPosts = allPostsListing
    .filter(p => p.id !== post.id && p.category === post.category)
    .slice(0, 3)

  // Get recent posts from the listing
  const recentPosts = allPostsListing.slice(0, 3)
    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-[#1a3c61] text-white">
        <div
          ...
          style={{
            backgroundImage: `url('${post.featured_image || '/default-blog-hero.jpg'}')`,
            ...
            <div className="lg:w-2/3">
              ...
              <div className="bg-white rounded-lg shadow-md p-8 mt-8">
                <div className="flex items-start">
                  ...
                  <div className="ml-6">
                    <h3 className="text-xl font-bold text-[#1a3c61]">{post.author?.name || "Khanna and Associates"}</h3>
                      ...
                      <div key={relatedPost.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <div className="p-4">
                          <p className="text-gray-500 text-xs mb-2">
                            {relatedPost.created_at ? format(relatedPost.created_at.toDate(), "MMMM dd, yyyy") : ""}
                          </p>
                    ...
                    <li key={recentPost.id} className="flex items-start">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-gray-500 text-xs mt-1">
                          {recentPost.created_at ? format(recentPost.created_at.toDate(), "MMMM dd, yyyy") : ""}
                        </p>
                      </div>
                    </li>
                  ))}
